...
rbr_files <- list.files('D:/Analyses/ladder_fuel_full_process_take3/filtered', 
                        pattern = '210218_RBR_3x3avg_rf_predict.csv',
                        full.names = T)

wrap_out <- 'D:/Analyses/ladder_fuel_full_process_take3/filtered/ladder-fuel_linear-regression_filtered_210218.png'
...
rbr$rbr_class <-
  cut(
    ...
    label = c('Very Low', 'Low', 'Medium', 'High')
  )
...
rbr$rbr_class <- factor(rbr$rbr_class, levels = c("High", "Medium", "Low", "Very Low"))
...
wrap_tls <- ggplot(data = rbr, aes(y=rbr_actual)) +
  ...
  annotate('text', x=330, y=7, label='very\nlow', angle=90, size=7, color='grey50')+
...
wrap_als <- ggplot(data = rbr, aes(y=rbr_actual)) +
  ...
  annotate('text', x=80, y=340, label='low', size=7, color='grey50')+
  annotate('text', x=215, y=340, label='medium', size=7, color='grey50')+
  annotate('label', x=215, y=20, label='y = 1.41x + -59.342\nRsq = 0.81\np<0.001', size=5)+
  ggtitle("ALS")+
  theme(legend.position = 'none', axis.title = element_blank())
wrap_als
# 
...
wrap_zeb <- ggplot(data = rbr, aes(y=rbr_actual)) +
  ...
  annotate('text', x=80, y=340, label='low', size=7, color='grey50')+
  annotate('text', x=215, y=340, label='medium', size=7, color='grey50')+
  annotate('label', x=215, y=20, label='y = 1.45x + -57.22\nRsq = 0.84\np<0.001', size=5)+
  ggtitle("MLS")+
  theme(legend.position = 'none', axis.title = element_blank())
wrap_zeb
...
wrap_uas <- ggplot(data = rbr, aes(y=rbr_actual)) +
  ...
  annotate('text', x=80, y=340, label='low', size=7, color='grey50')+
  annotate('text', x=215, y=340, label='medium', size=7, color='grey50')+
  annotate('label', x=215, y=20, label='y = 1.32x + -40.13\nRsq = 0.84\np<0.001', size=5)+
  ggtitle("UAS")+
  theme(legend.position = 'none', axis.title = element_blank())
wrap_uas
# 
...
wrap_banner <- ggplot(data = rbr, aes(y=rbr_actual)) +
  ...
  annotate('text', x=80, y=340, label='low', size=7, color='grey50')+
  annotate('text', x=215, y=340, label='medium', size=7, color='grey50')+
  annotate('label', x=215, y=20, label='y = 1.35x + -50.5\nRsq = 0.81\np<0.001', size=5)+
  ggtitle("Banner")+
