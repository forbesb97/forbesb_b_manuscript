...
# Description:
#
# Computes ladder fuel metrics as defined by TLS from TLS, HMLS (zeb), ALS, UAS
# 
#
# ==============================================================================
#
...
uav_las_files <- list.files(uav_las_folder, pattern = '.las', full.names = TRUE)
...
out_file <- 'D:/Analyses/ladder_fuel_full_process_take4/not_filtered/c1_ladder-fuels_metrics_tls_zeb_als_uas_210326.csv'
out_file_filtered <- 'D:/Analyses/ladder_fuel_full_process_take4/filtered_zero_and_above/c1_ladder-fuels_metrics_tls_zeb_als_uas_filtered_210326.csv'
...
for (tls_file in tls_las_files) {
  ...
  tls_metric_filter <- as_tibble(tls_las$Z) %>% 
    rename(Z='value') %>%
    summarize(
      tls_ladder_fuel_1to2 = (sum(Z > 1 & Z <= 2) / sum(Z >= 0 & Z <= 2)),
      tls_ladder_fuel_1to3 = (sum(Z > 1 & Z <= 3) / sum(Z >= 0 & Z <= 3)),
      tls_ladder_fuel_1to4 = (sum(Z > 1 & Z <= 4) / sum(Z >= 0 & Z <= 4)),
      tls_ladder_fuel_7to8 = (sum(Z > 7 & Z <= 8) / sum(Z >= 0 & Z <= 8))
    ) %>%
    add_column(campaign, plot, .before = 1)
...
for (als_file in als_las_files) {
  ...
  als_metric_filter <- as_tibble(als_las$Z) %>% 
    rename(Z='value') %>%
    summarize(
      als_ladder_fuel_1to2 = sum(Z > 1 & Z <= 2) / sum(Z >= 0 & Z <= 2),
      als_ladder_fuel_1to3 = sum(Z > 1 & Z <= 3) / sum(Z >= 0 & Z <= 3),
      als_ladder_fuel_1to4 = sum(Z > 1 & Z <= 4) / sum(Z >= 0 & Z <= 4),
      als_ladder_fuel_7to8 = sum(Z > 7 & Z <= 8) / sum(Z >= 0 & Z <= 8)
    ) %>%
    add_column(campaign, plot, .before = 1)
...
for (uav_file in uav_las_files) {
  ...
  uav_metric_filter <- as_tibble(uav_las$Z) %>% 
    rename(Z='value') %>%
    summarize(
      uav_ladder_fuel_1to2 = sum(uav_las$Z > 1 & uav_las$Z <= 2) / sum(uav_las$Z >= 0 & uav_las$Z <= 2),
      uav_ladder_fuel_1to3 = sum(uav_las$Z > 1 & uav_las$Z <= 3) / sum(uav_las$Z >= 0 & uav_las$Z <= 3),
      uav_ladder_fuel_1to4 = sum(uav_las$Z > 1 & uav_las$Z <= 4) / sum(uav_las$Z >= 0 & uav_las$Z <= 4),
      uav_ladder_fuel_7to8 = sum(uav_las$Z > 7 & uav_las$Z <= 8) / sum(uav_las$Z >= 0 & uav_las$Z <= 8)
    ) %>%
    add_column(campaign, plot, .before = 1)
  
  uav_combine_filter <- uav_combine_filter %>%
    add_row(uav_metric_filter)
...
for (zeb_file in zeb_las_files) {
  ...
  zeb_metric_filter <- as_tibble(zeb_las$Z) %>% 
    rename(Z='value') %>%
    summarize(
      zeb_ladder_fuel_1to2 = sum(Z > 1 & Z <= 2) / sum(Z >= 0 & Z <= 2),
      zeb_ladder_fuel_1to3 = sum(Z > 1 & Z <= 3) / sum(Z >= 0 & Z <= 3),
      zeb_ladder_fuel_1to4 = sum(Z > 1 & Z <= 4) / sum(Z >= 0 & Z <= 4),
      zeb_ladder_fuel_7to8 = sum(Z > 7 & Z <= 8) / sum(Z >= 0 & Z <= 8)
    ) %>%
    add_column(campaign, plot, .before = 1)
